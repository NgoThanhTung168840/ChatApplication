...
public class MessageController {
    ...
    public void mainChannel(InputTransportDTO dto, @Header("simpSessionId") String sessionId) {
        ActionTransport action = dto.getAction();
        switch (action) {
            ...
            case FETCH_GROUP_MESSAGES:
                if (!dto.getGroupUrl().equals("")) {
                    ...
                    if (dto.getMessageId() == -1) {
                        resMessages.setAction(ActionTransport.FETCH_GROUP_MESSAGES);
                    } else {
                        resMessages.setAction(ActionTransport.ADD_CHAT_HISTORY);
                    }
            ...
            case MARK_MESSAGE_AS_SEEN:
                if (!"".equals(dto.getGroupUrl())) {
                    ...
                    MessageUser messageUser = seenMessageService.findByMessageId(messageId, dto.getUserId());
                    if (messageUser == null) break;
                    messageUser.setSeen(true);
                    seenMessageService.saveMessageUserEntity(messageUser);
                }
                break;
            case LEAVE_GROUP:
                if (!dto.getGroupUrl().equals("")) {
                    ...
                    leaveResponse.setAction(ActionTransport.LEAVE_GROUP);
            ...
            case CHECK_EXISTING_CALL:
                ...
                for (String key : usersIndexedByRoomId.keySet()) {
                    if (key.contains(dto.getGroupUrl())) {
                        outputTransportDTO.setAction(ActionTransport.CALL_IN_PROGRESS);
                        this.messagingTemplate.convertAndSend("/topic/user/" + dto.getUserId(), outputTransportDTO);
                        break;
                    }
                }
                outputTransportDTO.setAction(ActionTransport.NO_CALL_IN_PROGRESS);
                this.messagingTemplate.convertAndSend("/topic/user/" + dto.getUserId(), outputTransportDTO);
                break;
            default:
                break;
    ...
    public void webRtcChannel(@DestinationVariable String roomUrl, RtcTransportDTO dto) {
        RtcAction action = dto.getAction();
        switch (action) {
            case INIT_ROOM -> {
                ...
                OutputTransportDTO outputTransportDTO = new OutputTransportDTO();
                outputTransportDTO.setAction(ActionTransport.CALL_INCOMING);
            ...
            case SEND_ANSWER -> {
                ...
                rtcTransportDTO.setAction(RtcAction.SEND_ANSWER);
            ...
            case JOIN_ROOM -> {
                ...
                RtcTransportDTO rtcTransportDTO = new RtcTransportDTO();
                rtcTransportDTO.setUserId(dto.getUserId());
                rtcTransportDTO.setAction(RtcAction.SEND_OFFER);
            ...
            case ICE_CANDIDATE -> {
                RtcTransportDTO rtcTransportDTO = new RtcTransportDTO();
                rtcTransportDTO.setUserId(dto.getUserId());
                rtcTransportDTO.setAction(RtcAction.ICE_CANDIDATE);
            ...
            case LEAVE_ROOM -> {
                ...
                if (hostListsIndexedByRoomUrl.size() == 0) {
                    ...
                    OutputTransportDTO outputTransportDTO = new OutputTransportDTO();
                    outputTransportDTO.setAction(ActionTransport.END_CALL);
    ...
    public void getAndSaveMessage(int userId, String groupUrl, String message) {
        ...
        Message messageEntity = new Message(userId, groupId, TypeMessage.TEXT.toString(), message);
        ...
        dto.setAction(ActionTransport.NOTIFICATION_MESSAGE);
    ...
    public WrapperMessageDTO getConversationMessage(String url, int messageId) {
        ...
        if (url != null) {
            ...
            List<Message> newMessages = messageService.findByGroupId(groupId, messageId);
            int lastMessageId = newMessages != null && newMessages.size() != 0 ? newMessages.get(0).getId() : 0;
            List<Message> afterMessages = messageService.findByGroupId(groupId, lastMessageId);
            if (newMessages != null) {
